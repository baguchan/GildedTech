...
public abstract class AbstractWorkerAllay extends PathfinderMob implements InventoryCarrier, IHostMob {
    ...
    public boolean isPlayerFriendly(Player player) {
        return this.getBrain().getMemory(MemoryModuleType.LIKED_PLAYER).isPresent() ? this.getBrain().getMemory(MemoryModuleType.LIKED_PLAYER).get() == player.getUUID() : false;
    }
