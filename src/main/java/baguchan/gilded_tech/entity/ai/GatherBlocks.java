package baguchan.gilded_tech.entity.ai;
...
public class GatherBlocks extends Behavior<AbstractWorkerAllay> {
    ...
    private boolean isGatherable(BlockState blockState, ServerLevel level, BlockPos pos, AbstractWorkerAllay mob) {
        return !blockState.isAir() && blockState.getDestroySpeed(level, pos) >= 0.0 && blockState.getDestroySpeed(level, pos) <= 10F && EventHooks.getMobGriefingEvent(level, mob);
    }
}